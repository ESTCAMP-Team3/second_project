...
def predict_stock(symbol):
    ...
    try:
        ...
        if needs_training:
            logger.info(f"[{symbol}] 모델 학습 시작 - Period: {period}")
        ...
        # 6. 예측 수행
        try:
            logger.info(f"[{symbol}] {days}일 예측 시작")
            ...
            for _, row in predictions.iterrows():
                predictions_list.append({
                    ...
                    '하한가격': round(float(row['하한가격']), 2),
                    '상한가격': round(float(row['상한가격']), 2)
                })

            logger.info(f"[{symbol}] 예측 완료 - {len(predictions_list)}개 결과")
            ...
            response_data = {
                ...
                'model_info': {
                    ...
                    'training_data_points': model_info['training_data_points'] if model_info else 0,
                    'prediction_days': days,
                    'changepoints': model_info['changepoints'] if model_info else 0,
                    'seasonalities': model_info['seasonalities'] if model_info else []
                },
